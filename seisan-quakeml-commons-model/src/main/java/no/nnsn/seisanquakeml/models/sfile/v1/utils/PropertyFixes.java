...
public class PropertyFixes {
    ...
    public static String fixLineFormat(PropertyObject obj, Boolean rightAlignment) {
        ...
        return fixLineSpace(obj, rightAlignment);
    }
        ...
        if (currentLength != expectedLength) {
            ...
            for (int i = 1; i <= expectedLength; i++) {
                if ((expectedLength - valSplit.length) >= i) {
                    emptySpace.append(wSpace);
                } else if ((valSplit.length - expectedLength) > 0 ) {
                    newVal.append(valSplit[i - 1]);
                } else {
                    int diff = expectedLength - valSplit.length;
                    newVal.append(valSplit[i - 1 - diff]);
                }
            }
            return emptyFirst ? (emptySpace + newVal.toString()) : (newVal.toString() + emptySpace);
        } else {
            return obj.getValue();
        }
    }
    ...
    private static void fixDoubleFormat(PropertyObject obj) {
        ...
        if (value.contains(".")) {
            ...
            if (decimals > 0) {
                ...
                int iNum = (int) Math.round(d);
                obj.setValue(Integer.toString(iNum));
            }
            ...
            if (valueLengthSize > objLengthSize) {
                ...
                if ((before + 2) <= objLengthSize && value.contains(".")) {
                    format.append("#".repeat(before));
                    format.append(".");

                    format.append("#".repeat(Math.max(0, (objLengthSize - before - 1))));
        ...
        if (obj.getValue().contains(".")) {
            ...
            if (valueLengthSize > objLengthSize) {
                ...
                if (midFormatCount > 0) {
                    ...
                    String format = initialFormat + "#".repeat(midFormatCount) +
                            endFormat;
